...
public class mxCellCodec extends mxObjectCodec
{
	...
	//Decode uid and graph-name into cell
  public Object afterDecode(mxCodec dec, Node node, Object obj)
  {
    if (obj instanceof mxCell) {
      mxCell cell = (mxCell) obj;
      if (StringUtils.isNotEmpty(((Element)node).getAttribute("uid"))){
        cell.setUid(((Element)node).getAttribute("uid"));
      }
      if (StringUtils.isNotEmpty(((Element)node).getAttribute("name"))){
        cell.setName(((Element)node).getAttribute("name"));
      }
    }
    return obj;
  }
