...
public class GraphProcess {
    ...
    public GraphProcess(mxGraphModel graphModel, Vertx vertx) {
        super();

        this.jsonArrayGraph = makeJsonArrayGraph(graphModel, vertx);
    }

    private JsonArray makeJsonArrayGraph(mxGraphModel graphModel, Vertx vertx) {
        final SharedData sharedData = vertx.sharedData();
        JsonArray ja = new JsonArray();
        mxCell cell0 = (mxCell) graphModel.getRoot();
        if (cell0.getData() != null && cell0.getData().toString().trim().length() > 1) {
            LocalMap<String, JsonObject> dataSource = sharedData.getLocalMap(cell0.getUid().toLowerCase() + ".data.source");
            dataSource.clear();
            JsonObject dat = new JsonObject(cell0.getData().toString());
            dat.forEach(item -> {
                dataSource.put(item.getKey(), (JsonObject) item.getValue());
            });
            JsonObject jo = new JsonObject();
            jo.put("id", cell0.getId());
            jo.put("type", "data.source");
            jo.put("data", new JsonObject(cell0.getData().toString()));
            ja.add(jo);
        }
