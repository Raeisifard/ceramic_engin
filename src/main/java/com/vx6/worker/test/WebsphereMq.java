...
public class WebsphereMq extends AbstractVerticle {
    private static final Logger log = LoggerFactory.getLogger(WebsphereMq.class);
    ...
    private <T> void processMessage(Message<T> tMessage) {
        ...
        boolean readAhead = false;//config.getBoolean("readAhead");
        ...
        try {
            ...
            if (e instanceof MQException) {
                tMessage.fail(((MQException) e).getCompCode(), new JsonObject().put("testStatus", "failed").put("reason", ((MQException) e).reasonCode).toString());
            } else {
