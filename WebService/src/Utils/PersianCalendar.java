...
public class PersianCalendar implements Serializable  {
    ...
    public  static Date getLastOfPersianMonthDate(Date date){
        String pToday = PersianCalendar.getPersianDate(date);
        int pToMonth = PersianCalendar.getPersianMonth(date)+1;
        int pToYear = PersianCalendar.getPersianYear(date);
        if (pToMonth==13) {
            pToMonth=1;
            pToYear++;
        }
        Calendar c=PersianCalendar.getGregorianCalendar(pToYear, pToMonth, 1);
        c.add(Calendar.DATE, -1);
        System.out.println("date  = " + date+"  --->  "+ c.getTime());
        System.out.println("date  = " + pToday+"  --->  "+PersianCalendar.getPersianDate( c.getTime()));
        return   c.getTime();
