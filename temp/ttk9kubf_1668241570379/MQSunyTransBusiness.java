...
public class MQSunyTransBusiness extends MasterVerticle {
    ...
    public void process(Message msg) {
        ...
        String equalRialAmount = Utils.Library.getPureNumStr(tran.getString("EqualRialAmount"));   // مبلغ معدل ارزی به ریال
        // clear garbage chars
        String cardPan1 = tran.getString("CARD_PAN1");
        String cardPan2 = tran.getString("CARD_PAN2");
        if ((cardPan2.length() < 16) || (!Utils.Library.isNumber(cardPan2) || (cardPan2.equals("0000000000000000"))))
            cardPan2 = "";
        String transIndicator = tran.getString("TransIndicator", "");
        String cardNo;
        if (transIndicator.equals("C"))
            cardNo = cardPan2;
        else
            cardNo = cardPan1;
        ...
        if (chequeStmtCodes.contains(stmtCode)) {
            ...
            if (module.startsWith("C56")) {
                amount = optionalInfo.trim();
                if (!amount.isEmpty())
                    amount = String.valueOf(Long.parseLong(amount));
            }
        }
        ...
        if (module.startsWith("RW39")) {//  0000416735 70
            chequeNumber = variableData.substring(1, 11);
        ...
        if (module.equals("DC68") || module.equals("FC68") || module.equals("GC68") || module.equals("ZC68")
                || module.equals("DC77") || module.equals("FC77") || module.equals("GC77") || module.equals("ZC77")
                || module.equals("ZC51") || module.equals("DC51") || module.equals("FC51") || module.equals("DG70")) {
            variableData = variableData.trim();
            if (variableData.length() == 11 && variableData.startsWith("09")) {
                forcedPhoneNo = variableData;
            } else {
                String trimmedComment3 = tran.getString("Comment3").trim();
                if (trimmedComment3.length() == 11 && trimmedComment3.startsWith("09"))
                    forcedPhoneNo = trimmedComment3;
            }
        }
        ...
        if (module.equals("ACH0") && stmtCode.equals("399")) {
            chequeNumber = variableData.trim();
        }
        String extraData = "";
        if (module.equals("GH00") && optionalInfo.trim().length() > 3) {//This is done just for BSI
            extraData = optionalInfo.trim().substring(3);
            tran.put("extraData", extraData);
        }
        String userId = tran.getString("UserID");
        if (module.equals("ACH0") && userId.equals("ACHXBSIR")) {
            extraData = optionalInfo.trim();
            tran.put("extraData", extraData);
        }
        if (transIndicator.trim().isEmpty())
            transIndicator = "N";
        String transId = module + transIndicator;
        if (transId.equals("BH05C") && stmtCode.equals("670")) {
            eb.publish(addressBook.getError(), body, addressBook.getDeliveryOptions(msg).addHeader("cause", "BLOCK"));
            errorOutboundCount++;
        }
        String uniqKey;
        ...
        if (isNotBlank(forcedPhoneNo)) {
            ...
            publishOut(0, body, addressBook.getDeliveryOptions(msg).addHeader("cause", "NO_PHONE"));
            return;
        }
        tran.put("cardPan2", cardPan2);
        ...
        if (isNotBlank(cardPan2))
            tran.put("DestCardNo", cardPan2.substring(13));
        ...
        tran.put("TranIndicator", transIndicator);
