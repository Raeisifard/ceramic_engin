...
public class Tran2Dom extends MasterVerticle {
    ...
    public void process(Message msg) {
        ...
        try {
            ...
            String command = commandXMLConfigElement.getName().toUpperCase().replace("-","_");
            joTran.put("COMMAND", command);
            List<org.jdom2.Attribute> attributes = commandXMLConfigElement.getAttributes();
            for (org.jdom2.Attribute attribute : attributes) {
                joTran.put(attribute.getName().toUpperCase().replace("-","_"), attribute.getValue());
            }
        ...
        eb.publish(addressBook.getResult(), new JsonObject().put("tran", joTran)
            .put("rawXml", transRaw).put("channel", msg.headers().get("type")).put("tranType", "xml"));
    }
}